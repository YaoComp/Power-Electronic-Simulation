...
/* Block states (default storage) for system 'PI Controller' ('LibraryForCodeGeneration:135') */
typedef struct {
  float Delay_DSTATE;             /* 'Delay' ('LibraryForCodeGeneration:244') */
} DW_PI_Controller_T;

/* Zero-crossing (trigger) state for system 'PI Controller' ('LibraryForCodeGeneration:135') */
typedef struct {
  ZCSigState Delay_Reset_ZCE;     /* 'Delay' ('LibraryForCodeGeneration:244') */
} ZCE_PI_Controller_T;
